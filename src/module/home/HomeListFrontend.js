...
import PostFrontendItem from "module/post/PostFrontendItem";
...
const HomeListFrontendStyles = styled.div`
  .layout {
    display: grid;
...
const HomeListFrontend = () => {
  ...
  useEffect(() => {
    ...
    const queries = query(
      ...
      where("status", "==", 1),
      where("hot", "==", false),
      where("category.slug", "==", "frontend-developer"),
      limit(3)
    );
  ...
  const [...other] = posts;
  return (
    <HomeListFrontendStyles className="home-block">
    ...
    </HomeListFrontendStyles>
  );
};

export default HomeListFrontend;
